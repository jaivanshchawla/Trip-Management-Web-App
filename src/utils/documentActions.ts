 ...
 * @param fileUrl - The exact S3 file URL from database (no modifications)
 * @returns Promise resolving to the presigned URL
 * @throws Error with specific message for different failure cases
 */
async function getShareableUrl(fileUrl: string): Promise<string> {
  try {
 ...
 * @returns Promise resolving to CopyResult with success status and message
 */
 ...
 * @param filename - The name of the document file
 * @returns Promise resolving to ShareResult with success status and fallback requirement
 */
