 ...
 * @param fileUrl - The S3 file URL from database (exact URL, no modifications)
 * @param expiresIn - Expiration time in seconds (default: 48 hours)
 * @returns Presigned URL for temporary public access
 */
export async function getPresignedUrl(
  fileUrl: string,
  expiresIn: number = 172800 // 48 hours in seconds
): Promise<string> {
  try {
    const bucketName = process.env.AWS_S3_BUCKET_NAME!;
